...
export default function ChildTimeline({ child, schedules, timetable, date, onEdit, onDelete }: TimelineProps) {
  ...
  return (
    <div className="glass-card !p-4 border-none shadow-sm space-y-4">
      {/* Header Widget */}
      <div className="flex items-center justify-between pb-3 border-b border-gray-50 mb-2">
        <div className="flex items-center gap-2">
           <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm ${isJeum ? 'bg-blue-50' : 'bg-emerald-50'}`}>
              {emoji}
           </div>
           <div>
              <h3 className="type-section leading-none">{name}이 타임라인</h3>
              <p className="type-caption text-gray-400 mt-0.5">{daySchedules.length}개의 일정 · {timetable.length}교시 수업</p>
           </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {timetable.length > 0 && (
          <div className="bg-gray-50/50 rounded-xl p-3 border border-gray-100/50">
             <p className="type-caption text-gray-400 mb-2 flex items-center gap-1.5 px-1">
                <BookOpen size={10} /> 학교 시간표
             </p>
             <div className="flex flex-wrap gap-2">
                {timetable.sort((a,b) => Number(a.period) - Number(b.period)).map((t, idx) => (
                   <div key={idx} className="flex flex-col items-center min-w-[45px] bg-white rounded-lg p-2 shadow-sm border border-gray-100">
                      <span className="type-caption text-blue-500 mb-0.5">{t.period}교시</span>
                      <span className="type-body font-black text-gray-900">{t.subject}</span>
                   </div>
                ))}
             </div>
          </div>
            ...
            {daySchedules.map((sched) => {
              ...
              return (
                <div key={sched.id} className="animate-fade-in">
                  <div className={`p-4 rounded-xl border transition-all ${isExpanded ? 'bg-white border-gray-200 shadow-sm' : 'bg-gray-50/50 border-gray-50 hover:bg-gray-50'}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0" onClick={() => prepItems.length > 0 && setExpandedId(isExpanded ? null : sched.id)}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`badge-mini ${isJeum ? 'bg-blue-100 text-blue-600' : 'bg-emerald-100 text-emerald-600'}`}>
                            {CATEGORY_LABELS[sched.category]}
                          </span>
                          {prepItems.length > 0 && (
                            <span className="badge-mini bg-orange-100 text-orange-600 flex items-center gap-1">
                               <Backpack size={10} /> {prepItems.length}
                            </span>
                          )}
                        </div>
                        <h4 className="type-body font-black text-gray-900 mb-2 group-hover:text-blue-500 transition-colors">
                          {sched.title}
                        </h4>
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                          <span className="type-time flex items-center gap-1">
                             <Clock size={10} /> {sched.start_time} - {sched.end_time}
                          </span>
                          {sched.location && (
                             <span className="type-time flex items-center gap-1 max-w-[120px] truncate">
                      ...
                      <div className="flex flex-col gap-1.5 self-center">
                         <button 
                           ...
                           className="w-8 h-8 rounded-lg bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-blue-500 hover:border-blue-100 transition-all shadow-sm"
                         >
                            <Edit3 size={14} />
                         </button>
                      </div>
